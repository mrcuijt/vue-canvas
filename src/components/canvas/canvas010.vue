...
export default {
  ...
  created () {
    bus.on('rangeEvent', this.rangeEvent)
  },
  data () {
    return {
      width: 350,
  ...
  methods: {
    ...
    fill: function () {
      ...
      this.datas.sort(this.compareValues('index'))
    ...
    rangeEvent: function () {
      this.datas[3].index = 1
      this.datas[2].index = 2
      this.datas[1].index = 3
      this.datas[0].index = 4
      this.arf = window.requestAnimationFrame(this.fill)
    }
  },
  mounted () {
