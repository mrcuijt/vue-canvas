<template>
  <div>
    <button
      id="btn1"
      @click="btnUpdate([2000,200,30,1])"
    >数据1</button>&nbsp;&nbsp;
    <button
      id="btn2"
      @click="btnUpdate([800,700,500,100])"
    >数据2</button>&nbsp;&nbsp;
    <button
      id="btn3"
      @click="btnUpdate([1500,2300,300,50])"
    >数据3</button>&nbsp;&nbsp;
  </div>
  <canvas
    id="canvas"
...
// 动画渲染速度处理
class Rect {
  constructor(index, space, color, reqTotal) {
    this.maxWidth = 0
    this.maxHeigth = 0
    this.x = 0
    this.y = 0
...
export default {
  ...
  data () {
    return {
      ...
      reqMax: 1200,
      datas: [
        new Rect(1, 10, 'rgb(255,0,0)', 200), // red
        new Rect(2, 10, 'rgb(0,255,0)', 300), // green
        new Rect(3, 10, 'rgb(0,0,255)', 500), // blue
        new Rect(4, 10, 'rgb(255,255,0)', 800) // yellow
      ]
    }
  },
  created () {
